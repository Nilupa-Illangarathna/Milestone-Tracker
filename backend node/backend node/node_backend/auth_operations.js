...
async function login_post_method(username, password) {
  ...
  async function signup_post_method(name, age, username, email, mobile, password) {
    ...
    const { data: existingUser, error } = await crud('goalkeepingapp_users', 'read', {
      or: [
        { username },
        { email }
      ]
    ...
    const { data: createdUser, error: createError } = await crud('goalkeepingapp_users', 'create', {}, newUser);
  ...
  async function sendOTP(contact, method, isNumericOTP, otpDigitCount) {
    ...
    const { data: existingOTP, error: existingOTError } = await supabase.from('goalkeepingapp_otpdata').select('id').eq('contact', contact).eq('method', method);
    ...
    if (existingOTP && existingOTP.length > 0) {
      ...
      await supabase.from('goalkeepingapp_otpdata').update({ otp: otp }).eq('id', existingOTPId);
      ...
      const { data: createdOTP, error: createError } = await supabase.from('goalkeepingapp_otpdata').insert([{ method, contact, otp: otp }]);
...
// verifyOTP
  async function verifyOTP(method, contact, otp) {
    ...
    const { data: otpData, error } = await supabase.from('goalkeepingapp_otpdata').select('otp').eq('method', method).eq('contact', contact);
  ...
  async function reset_pass_function(method, contact, password, res){
    ...
    const { data: user, error } = await supabase.from('goalkeepingapp_users')
        .select('*')
    ...
    if (user) {
        ...
        const { data: updatedUser, updateError } = await supabase.from('goalkeepingapp_users')
            .update({ password })
            .eq('id', user.id);
